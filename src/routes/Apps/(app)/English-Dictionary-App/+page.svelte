...
<section class="mx-auto my-10 flex max-w-3xl flex-col justify-center">
	{#if typeof currentMeaning === 'object' && 'title' in currentMeaning}
		...
		>
			<NoWord />
			<h2 class="text-xl font-semibold">{currentMeaning.title}</h2>
			<p class="text-md mt-2">{currentMeaning.message}</p>
			<p class="mt-1 text-sm text-red-600 dark:text-red-400">{currentMeaning.resolution}</p>
		</div>
	{:else if typeof currentMeaning === 'object'}
		<article class="rounded-lg bg-white p-4 shadow-md dark:bg-gray-900">
			...
			{#if currentMeaning.phonetic}
				...
				<p class="mb-2 text-lg font-semibold text-green-600 dark:text-green-400">
					Phonetic: <span class="rounded-md bg-gray-200 px-2 py-1 dark:bg-gray-700"
						>{currentMeaning.phonetic}</span
					>
				</p>
			{/if}
			{#each currentMeaning.phonetics || [] as phonetic}
				{#if phonetic.audio}
					<div
				...
				{/if}
			{/each}
			{#each currentMeaning.meanings as meaning}
				<section>
			...
			<footer>
				...
				>
					License: {currentMeaning.license.name} -
					<a
						href={currentMeaning.license.url}
						class="text-blue-500 hover:underline dark:text-blue-400"
						target="_blank"
						rel="noopener">{currentMeaning.license.url}</a
					>
				</p>
			</footer>
		</article>
		{#if currentMeaning.origin}
			<p class="mt-4 text-lg font-semibold text-green-600 dark:text-green-400">
				Origin: {currentMeaning.origin}
			</p>
		{/if}
	{/if}
</section>
